...
class PurchasesTableViewModel : ViewModel() {

    private val _tableData = mutableStateListOf<List<String>>()
    val tableData: List<List<String>> = _tableData
    fun loadPurchasesTable(userId: Int) {

        val columnsToInclude = listOf("isbn", "title", "price", "times_purchased")

        viewModelScope.launch(Dispatchers.IO) {
            try {
                val url = URL("http://undcemcs02.und.edu/~collin.l.ferguson/457/2/mobile_database.php")
                val postData = "action=get_purchases&user_id=$userId"
